...
export const addExpense = async ({
  groupId, paidBy, amount, currency, description, category = 'general',
  splitType = 'equal', splitData = null, memberIds,
  isRecurring = false, recurInterval = null, recurNextDate = null, recurParentId = null,
}) => {
  ...
  const { data: expense, error: eErr } = await supabase
    .from('expenses')
    .insert({
      group_id: groupId, paid_by: paidBy, amount, currency, amount_usd: amountUsd,
      description, category, split_type: splitType,
      is_recurring: isRecurring,
      recur_interval: recurInterval || null,
      recur_next_date: recurNextDate || null,
      recur_parent_id: recurParentId || null,
    })
    .select()
    .single();
...
const calculateSplits = ({ expense, splitType, splitData, memberIds, paidBy }) => {
  ...
  if (splitType === 'equal') {
    ...
    if (!Array.isArray(splitData)) return splits;
    const totalShares = splitData.reduce((s, d) => s + (parseFloat(d.shares) || 1), 0);
    splitData.forEach(({ userId, shares }) => {
      const amountOwed = parseFloat(((expense.amount * (parseFloat(shares) || 1)) / totalShares).toFixed(2));
      splits.push({ userId, amountOwed: Number(userId) === Number(paidBy) ? 0 : amountOwed });
    });
  }
  ...
  const { data, error } = await supabase
    ...
    .select(`
      *,
      ...
      expense_comments (id, text, created_at, user_id, users!user_id(full_name))
    `)
    .eq('group_id', groupId)
...
const minimizeTransactions = (balances) => {
  ...
  while (i < creditors.length && j < debtors.length) {
    ...
    if (settle < 0.01) break;
    transactions.push({
      from: { telegram_id: debtors[j].telegram_id, full_name: debtors[j].full_name, ton_wallet: debtors[j].ton_wallet },
      to:   { telegram_id: creditors[i].telegram_id, full_name: creditors[i].full_name, ton_wallet: creditors[i].ton_wallet },
      amount: settle,
    });
...
export const settleDebt = async ({ groupId, fromUserId, toUserId, amount, currency = 'USD', method = 'manual', txHash = null }) => {
  const { data: expenseIds } = await supabase
    .from('expenses').select('id').eq('group_id', groupId).eq('paid_by', toUserId);
  ...
  if (expenseIds?.length) {
    ...
    await supabase
      ...
      .in('expense_id', ids).eq('user_id', fromUserId).eq('is_settled', false);
  }
...
export const deleteExpense = async (expenseId, requestingUserId) => {
  const { data: expense } = await supabase
    .from('expenses').select('paid_by, group_id').eq('id', expenseId).maybeSingle();
  if (!expense) throw new Error('EXPENSE_NOT_FOUND');

  const { data: member } = await supabase
    .from('group_members').select('role').eq('group_id', expense.group_id).eq('user_id', requestingUserId).maybeSingle();
